...
def psnr(img1, img2, is_masked=False):
    # return 20 * torch.log10(1.0 / torch.sqrt(mse(img1, img2)))

    mse_value = mse(img1, img2)
    
    if is_masked:
        num_nonzero = torch.count_nonzero(img1).item()
        mse_value = mse_value * img1.numel() /  num_nonzero
    
    epsilon = 1e-12
    return 20 * torch.log10(1.0 / torch.sqrt(mse_value + epsilon))
