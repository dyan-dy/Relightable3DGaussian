...
for i in $list; do
    python train.py --eval \
        ...
        --model_path output/human/${i}_debug_2/3dgs \
        --resolution 1 \
        --lambda_normal_render_depth 0.1 \
        --lambda_normal_smooth 0.1 \
        --lambda_mask_entropy 0.9 \
        --save_training_vis \
        --lambda_depth_var 1e-2  


    python eval_nvs.py --eval \
        -m output/human/${i}_debug_2/3dgs \
        -c output/human/${i}_debug_2/3dgs/chkpnt30000.pth

    python train.py --eval \
        -s /home/gdj592/code/lj/data/$i/ \
        -m output/human/${i}_debug_2/neilf \
        --resolution 1 \
        -c output/human/${i}_debug_2/3dgs/chkpnt30000.pth \
        --save_training_vis \
    ...
    python eval_nvs.py --eval \
        -m output/human/${i}_debug_2/neilf \
        -c output/human/${i}_debug_2/neilf/chkpnt40000.pth \
        -t neilf
done
